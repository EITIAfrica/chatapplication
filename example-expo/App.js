...
import { Bubble, ChatApp, SystemMessage } from 'chatapplication';
...
export default class App extends Component {
  ...
  onLoadEarlier = () => {
    ...
    setTimeout(() => {
      if (this._isMounted === true) {
        this.setState((previousState) => {
          return {
            messages: ChatApp.prepend(previousState.messages, earlierMessages),
            loadEarlier: false,
            isLoadingEarlier: false,
          };
        });
  ...
  onSend = (messages = []) => {
    ...
    this.setState((previousState) => {
      ...
      return {
        messages: ChatApp.append(previousState.messages, sentMessages),
        step,
      };
    });
    // for demo purpose
  ...
  botSend = (step = 0) => {
    ...
    if (newMessage) {
      this.setState((previousState) => ({
        messages: ChatApp.append(previousState.messages, newMessage),
      }));
    }
  };
  ...
  onReceive = (text) => {
    this.setState((previousState) => {
      return {
        messages: ChatApp.append(previousState.messages, {
